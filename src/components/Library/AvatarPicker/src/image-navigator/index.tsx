...
import { MAX_SIZE_MB, ACCEPT } from '../avatar-picker-dialog';
...
export class ImageNavigator extends Component<Props, State> {
  ...
  validateFile(imageFile: File): string | null {
    ...
    if (ACCEPT.indexOf(imageFile.type) === -1) {
      return translate('common.media.image-format-invalid-error');
    } else if (fileSizeMb(imageFile) > MAX_SIZE_MB) {
      return translate('common.media.image-size-too-large-error');
    }
    return null;
  }
  ...
  renderDragZone = () => {
    ...
    const dragZoneText =
      errorMessage ||
      this.props.translate('common.media.drag-and-drop-images-here');
    const dragZoneAlt =
      errorMessage || this.props.translate('common.media.upload-image');
      ...
      <ImageUploader>
        ...
        {isLoading ? null : (
          <div>
            <PaddedBreak>
              {errorMessage
                ? this.props.translate('common.media.try-again')
                : this.props.translate('common.media.or')}
            </PaddedBreak>
            <Button onClick={this.onUploadButtonClick} isDisabled={isLoading}>
              {this.props.translate('common.media.upload-photo')}
              <FileInput
                type="file"
                name="image-input"
                id="image-input"
